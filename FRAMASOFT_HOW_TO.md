# Grab binaries compiled by Framasoft

Just go to <https://packages.framasoft.org/projects/mattermost/> to get the binaries you want.

Follow the instructions on top of the page to verify the binaries you downloaded.

# How to use Framasoft’s patch to recompile Mattermost without users limit
...
The CI will compile mattermost. Note that it will not be available as an artifact.

You can use the step of [.gitlab-ci.yml](.gitlab-ci.yml) to compile Mattermost without using the CI.
...
The CI will publish the compiled mattermost binary through sftp.
